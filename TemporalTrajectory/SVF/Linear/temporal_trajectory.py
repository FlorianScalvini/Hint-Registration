...
class TemporalTrajectorySVF(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            forward_flow, backward_flow = self.regnet.velocity_to_flow(velocity=velocity)
            label_warped_source = self.regnet.warp(subject_t0['label'][tio.DATA].to(self.device).float(),
                                                   forward_flow)
            image_warped_source = self.regnet.warp(subject_t0['image'][tio.DATA].to(self.device).float(),
                                                   forward_flow)
