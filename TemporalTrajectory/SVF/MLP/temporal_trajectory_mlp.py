...
class TemporalTrajectoryMLPSVF(pl.LightningModule):
    ...
    '''
    def __init__(self, regnet: RegistrationModuleSVF, mlp_model : MLP, loss: str = 'mse', lambda_sim: float = 1.0,
    ...
    def forward(self, source: Tensor, target: Tensor, age: Tensor):
        ...
        forward_flow, backward_flow = self.regnet.velocity_to_flow(velocity * coef)
    ...
    def training_step(self, batch, batch_idx):
        ...
        velocity = self.regnet(self.subject_t0['image'][tio.DATA], self.subject_t1['image'][tio.DATA]) # Compute the velocity field
    ...
    def load_model(self, regnet_path: str, mlp_path: str):
        self.regnet.load_state_dict(torch.load(regnet_path))
...
class TemporalTrajectoryActiveLearningMLP(TemporalTrajectoryMLPSVF):
    def __init__(self, regnet: RegistrationModuleSVF, mlp_model : MLP, loss: str = 'mse', lambda_sim: float = 1.0,
        ...
        super().__init__(regnet, mlp_model, loss, lambda_sim, lambda_seg, lambda_mag, lambda_grad, num_classes, num_inter_by_epoch, save_path)
    ...
    def training_step(self, batch, batch_idx):
        ...
        for i in self.index_train_current_epoch:
            ...
            forward_flow, backward_flow = self.regnet.velocity_to_flow(velocity * coef)
            loss_intermediate_tensor += self.regnet.registration_loss(self.subject_t0, subject, forward_flow, backward_flow, lambda_sim=self.lambda_sim, lambda_seg=self.lambda_seg, lambda_mag=self.lambda_mag, lambda_grad=self.lambda_grad)
    ...
    def on_train_epoch_end(self):
        ...
        with torch.no_grad():
            ...
            if mean_all_dice > self.dice_max:
                ...
                print("New best dice:", self.dice_max)
                torch.save(self.regnet.state_dict(), self.save_path + "/regnet_best.pth")
