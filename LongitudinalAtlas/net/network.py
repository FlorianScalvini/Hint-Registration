...
from svf_modules import VecInt, SpatialTransformer
...
class RegistrationModelSVF(nn.Module):
    ...
    def forward(self, source: Tensor, target: Tensor, time: float=1.0) -> (Tensor, Tensor):
        x = torch.cat([source, target], dim=1) #.to(torch.float32)
        forward_velocity = self.model(x)
        forward_flow = self.vecint(forward_velocity * time)
