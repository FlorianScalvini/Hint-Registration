...
class SegmentationMetric(object):
    def __init__(self, num_classes):
        ...
        self.preds_flat = np.zeros(self.num_classes)
        self.targets_flat = np.zeros(self.num_classes)

    def update(self, preds, targets):
        result = np.zeros(self.num_classes)
        for label_index in range(self.num_classes):
            class_predictions = preds.eq(label_index)
            class_predictions = class_predictions.squeeze(1)  # remove channel dim
            class_labels = targets.eq(label_index)
            ...
            p_flat = class_predictions.view(class_predictions[0], -1)
            l_flat = class_labels.view(class_predictions[0], -1)
            self.preds_flat += p_flat.sum()
            self.targets_flat += l_flat.sum()
            self.tp[label_index] += (p_flat * l_flat).sum()
            negative = 1 - p_flat
            self.fn[label_index] = (negative * l_flat).sum()

    def precision_score(self, eps=1e-6):
        try:
            ps = (self.tp + eps) / (self.preds_flat + eps)
        except:
            ps = torch.tensor(np.NaN)
        return ps

    def dice_score(self, eps=1e-6):
        try:
            dc = ((2. * self.tp) + eps) / ((self.preds_flat + self.targets_flat) + eps)
        except ZeroDivisionError:
            dc = torch.tensor(np.NaN)
        return dc

    def recall_score(self, eps=1e-6):
        try:
            rs = (self.tp + eps) / ((self.tp + self.fn) + eps)
        except ZeroDivisionError:
            rs= torch.tensor(np.NaN)
        return rs
