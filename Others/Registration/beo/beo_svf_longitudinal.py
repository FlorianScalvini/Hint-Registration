...
class CustomRandomDataset(Dataset):
    """
    Custom PyTorch dataset.py that picks 3 different random elements from another dataset.py.

    Args:
        dataset (torchio.Dataset): The original dataset.py to sample from.
    """
    ...
    def __len__(self):
        #return math.comb(len(self.dataset.py),3) # the number of unique triplets
        return len(self.dataset)-2 # the number of triplets with two anchor points

    def __getitem__(self, idx):
        #random_indices = random.sample(range(0, len(self.dataset.py)), 3)
        indice_t0 = 0
        indice_t1 = 0
        ...
        #return [self.dataset.py[i] for i in random_indices]
