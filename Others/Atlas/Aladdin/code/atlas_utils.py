...
def get_train_valid_list_forward_atlas():
    ...
    with open('train.txt', 'r') as f_valid:
        valid_list = list(f_valid.read().splitlines())
    with open('train.txt', 'r') as f_test:
        test_list = list(f_test.read().splitlines())
...
def save_updated_atlas(atlas_img, atlas_seg, save_atlas_img_name, save_atlas_est_name, save_atlas_prob_name):
    ...
    atlas_img_np = atlas_img.detach().cpu().numpy()
    atlas_seg_np = atlas_seg.detach().squeeze().cpu().numpy()
    atlas_est_np = torch.max(atlas_seg, 1)[1].detach().cpu().numpy()
    ...
    o = tio.ScalarImage(tensor=atlas_img_np.squeeze(0),affine=tmp_sub["label"].affine)
    o.save(save_atlas_img_name)
    o = tio.LabelMap(tensor=atlas_est_np,
                     affine=tmp_sub["img"].affine)
    o.save(save_atlas_est_name)
