...
if __name__ == "__main__":
    ...
    experiment_name = 'DHCP_Aladdin_' + str(loss_name) \
                      ...
                      + '_network_lr_' + str(lr)
    train_model = SVF_resid(img_sz=np.array([128, 128, 128]), args=args)
    ...
    for epoch in range(max_epochs):
        ...
        for i, (src_imgs, src_segs, src_ids) in enumerate(SVFNet_train_dataloader):
            ...
            del svf_warped_atlas_imgs, pos_flow, pos_deform_field, neg_flow, neg_deform_field
        ...
        if epoch % args.save_per_epoch == (args.save_per_epoch - 1):
            with torch.set_grad_enabled(False):
                ...
                for _, (mean_src_imgs, mean_src_segs, _) in enumerate(SVFNet_val_dataloader):
                    ...
                    mean_warped_src_segs = bilinear(mean_src_segs.float(), mean_neg_deform_field_src.float())
                ...
                if dice_avg > best_score:
                    ...
                    for _, (update_src_imgs, update_src_segs, _) in enumerate(SVFNet_atlas_update_dataloader):
                        ...
                        update_warped_src_segs = bilinear(update_src_segs.float(), update_neg_deform_field_src)
