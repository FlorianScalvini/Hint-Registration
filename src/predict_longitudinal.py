...
from utils import get_cuda_is_available_or_cpu, create_directory, seg_map_error, map_labels_to_colors, write_namespace_arguments, subjects_from_csv
...
def test(args):
    ...
    model = OurLongitudinalDeformation(
        mode=args.mode, hidden_dim=args.mlp_hidden_dim, num_layers=args.mlp_num_layers, t0=args.t0, t1=args.t1,
        reg_model=RegistrationModuleSVF(model=monai.networks.nets.AttentionUnet(spatial_dims=3, in_channels=2,
                                                                                out_channels=3, channels=[4, 8, 16, 32],
                                                                                strides=[2, 2, 2]), int_steps=7))
