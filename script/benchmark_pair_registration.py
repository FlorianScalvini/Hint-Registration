...
def _compute_unigradicon(dataset_path, source_image_path, source_label_path, device, num_classes, orig_shape=False):
    model = UniGradIcon()
    return _compute_registration_dice_between_source_and_targets(model, dataset_path, source_image_path, source_label_path, (175, 175, 175), (221, 221, 221), num_classes, upsample=orig_shape, device=device)
